...
function projectCardHTML(p){
  ...
  return `<div class="card pcard rv ${hasImages(p) ? 'plated' : ''}" data-popen="${p.id}" draggable="true" data-kdrag="${p.id}" style="cursor:pointer;--c:${st[2]}">${imageBackdropHTML(p)}<div class="hd"><h3>${esc(p.name)}</h3><span class="row" style="gap:6px">${prBadge(p)}${stBadge(p)}</span></div><div class="muted" style="font-size:.85rem">${esc(p.description||'')}</div><div class="row">${(p.tags||[]).map(t=>`<span class="chip">${esc(t)}</span>`).join('')}</div>
    ...
    ${nodHeat(p)}<div class="mono">${ns.length} nods · last ${relDays(daysSince(ns[0]?.date))}${p.income?.current?` · ${fmtYen(p.income.current)}/mo`:''}</div>${p.link&&p.status==='completed'?`<a href="${esc(p.link)}" target="_blank" rel="noopener noreferrer" class="autolink mono" onclick="event.stopPropagation()">${esc(linkLabel(p.link))}</a>`:''}</div>`;
}
...
function ganttHTML(ps){
  ...
  rows.forEach(p => { const st = PSTATUS[p.status]||PSTATUS.idea; const phases = (p.phases||[]).filter(ph => ph.startDate || ph.endDate);
    ...
    phases.forEach((ph,i) => { const a = ph.startDate || ph.endDate, b = ph.endDate || ph.startDate; const pt = projectPhaseTasks(p, ph); const r = {done:pt.filter(t=>t.done).length, total:pt.length}; const w = Math.max(6, X(b)-X(a)); g += `<g class="g-bar" data-popen="${p.id}"><rect x="${X(a).toFixed(1)}" y="${y}" width="${w.toFixed(1)}" height="${phH}" rx="4" fill="${st[2]}" opacity=".85"/><rect x="${X(a).toFixed(1)}" y="${y}" width="${(w*(r.total?r.done/r.total:0)).toFixed(1)}" height="${phH}" rx="4" fill="#fff" opacity=".22"/><clipPath id="gc-${p.id}-${i}"><rect x="${X(a).toFixed(1)}" y="${y}" width="${w.toFixed(1)}" height="${phH}" rx="4"/></clipPath><text clip-path="url(#gc-${p.id}-${i})" x="${(X(a)+6).toFixed(1)}" y="${y+11.5}" style="fill:#1a1816;font-size:9px;font-family:var(--sans)">${esc(ph.name.length>28?ph.name.slice(0,27)+'…':ph.name)}${r.total?` · ${r.done}/${r.total}`:''}</text><title>${esc(ph.name)} · ${fmtDate(a,'med')} → ${fmtDate(b,'med')}</title></g>`; y += phH + 4; });
...
function openProjectPanel(id){
  ...
  const pn = openPanel(`<div class="mono">project · ${ns.length} nods · avg energy ${ns.length?avg(ns.map(n=>n.energy)).toFixed(1):'–'} · ${r.done}/${r.total} tasks</div><h2>${ed(`projects.#${p.id}.name`)}</h2><div class="muted">${ed(`projects.#${p.id}.description`,{multi:true,ph:'one or two sentences'})}</div>
    ...
    <div class="spec-grid"><div><div class="k">start</div>${ed(`projects.#${p.id}.startDate`,{ph:'YYYY-MM-DD',cls:'mono',date:true})}</div><div><div class="k">target</div>${ed(`projects.#${p.id}.targetDate`,{ph:'YYYY-MM-DD',cls:'mono',date:true})}</div></div>
    ...
    <div class="vp-sec"><div class="row between"><span class="sc">Phases &amp; tasks</span><button class="btn sm ghost" id="phAdd">+ phase</button></div>
      ...
      <div class="phases">${(p.phases||[]).map((ph,i) => { const phT = projectPhaseTasks(p, ph); const pr = {done:phT.filter(t=>t.done).length, total:phT.length}; const isOpen = openPh ? openPh===ph.id : i===0; return `<div class="phase ${isOpen?'open':''}" data-phase="${ph.id}">
        ...
        <div class="phase-body"><div class="phase-inner">
          ...
          <div class="tasks">${phT.map((t,ti) => { const dd = t.dueDate || t.day || ''; const due = dd ? daysBetween(today(), dd.slice(0,10)) : null; return `<div class="task ${t.done?'done':''}" data-task="${t.id}"><label><input type="checkbox" data-tdone="${ph.id}:${t.id}" ${t.done?'checked':''}></label><span class="task-text">${ph.movedAt ? ed(`tasks.#${t.id}.text`,{ph:'task'}) : ed(`projects.#${p.id}.phases.${i}.tasks.${ti}.text`,{ph:'task'})}</span><input type="date" class="inp task-due ${due!==null&&due<0&&!t.done?'over':''}" value="${dd?dd.slice(0,10):''}" data-tdue="${ph.id}:${t.id}" title="due date">${due!==null&&!t.done?`<span class="mono ${due<0?'due':''}">${due<0?`${-due}d over`:due===0?'today':`${due}d`}</span>`:''}<button class="del-x inline" data-tdel="${ph.id}:${t.id}" title="delete task">×</button></div>`; }).join('')||'<div class="faint" style="font-size:.78rem;padding:4px 0">no tasks yet</div>'}</div>
    ...
    <div class="vp-sec"><div class="row between"><span class="sc">Resources</span><button class="btn sm ghost" id="resAdd">+ resource</button></div>
      <div class="lvl-res">${(p.resources||[]).map((rs,ri)=>`<div class="res-row"><span class="res-type">${{link:'🔗',file:'📎',note:'📝'}[rs.type]||'▫'}</span>${rs.url?`<a class="res-link autolink" href="${esc(rs.url)}" target="_blank" rel="noopener noreferrer">${esc(rs.title||linkLabel(rs.url))}</a>`:`<span>${esc(rs.title||'untitled')}</span>`}<span class="status-pill" style="font-size:.6rem">${esc(rs.type)}</span><span class="res-edit"><span class="ed-wrap">${ed(`projects.#${p.id}.resources.${ri}.title`,{ph:'title',cls:'mono'})}</span><span class="ed-wrap">${ed(`projects.#${p.id}.resources.${ri}.url`,{ph:'https://… (optional for notes)',cls:'mono'})}</span><select class="sel" data-prestype="${ri}" style="width:auto;padding:1px 6px;font-size:.66rem;padding-right:22px">${['link','file','note'].map(t=>`<option ${rs.type===t?'selected':''}>${t}</option>`).join('')}</select><button class="del-x inline" data-presdel="${ri}" title="remove">×</button></span></div>`).join('')||'<div class="faint" style="font-size:.8rem">Links, files, notes.</div>'}</div></div>
    ...
    <div class="vp-sec"><span class="sc">Income stream</span><div class="spec-grid"><div><div class="k">revenue model</div>${ed(`projects.#${p.id}.income.model`,{ph:'freelance / product / subscriptions / patronage'})}</div><div><div class="k">current monthly (¥)</div>${ed(`projects.#${p.id}.income.current`,{ph:'0',cls:'mono',hook:'pnum:'+p.id})}</div><div><div class="k">target monthly (¥)</div>${ed(`projects.#${p.id}.income.target`,{ph:'0',cls:'mono',hook:'pnum:'+p.id})}</div></div>
      <div class="row between" style="margin-top:10px"><span class="k mono">milestones</span><button class="btn sm ghost" id="pMile">+ milestone</button></div>${(p.income.milestones||[]).map((m,i)=>`<div class="evidence-item"><span class="mono">${ed(`projects.#${p.id}.income.milestones.${i}.date`,{ph:'date',cls:'mono',date:true})}</span><span style="flex:1">${ed(`projects.#${p.id}.income.milestones.${i}.text`,{ph:'first user, first dollar, first referral…'})}</span><button class="tbtn" data-mdel="${i}">×</button></div>`).join('')||'<div class="empty">Small wins, recorded. None yet.</div>'}</div>
    ...
    <div class="vp-sec"><span class="sc">Nods</span><div class="nodlist">${ns.slice(0,40).map(n=>`<div class="nod"><span class="mono">${fmtDate(n.date,'med')}</span><span>${linkify(n.text)}${n.link?` <a class="autolink mono" href="${esc(n.link)}" target="_blank" rel="noopener noreferrer">↗</a>`:''}${n.image?`<div class="photo" style="width:60px;height:60px;margin-top:4px"><img src="${n.image}"></div>`:''}</span><span class="row" style="gap:6px"><span class="mono">${n.duration||''} · ${'●'.repeat(n.energy)}${'○'.repeat(5-n.energy)}</span><button class="tbtn" data-nodedit="${n.id}">edit</button><button class="del-x inline" data-noddel="${n.id}" title="delete nod">×</button></span></div>`).join('')||'<div class="empty">No nods yet. Show up once.</div>'}</div></div>
  ...
  pn.querySelector('#phAdd').onclick = () => { const ph = {id:uid(), name:`Phase ${(p.phases||[]).length+1}`, startDate:'', endDate:'', tasks:[]}; if(p.movedTo) ph.movedAt = new Date().toISOString(); p.phases.push(ph); if(typeof projectListsSync === 'function') projectListsSync(); S._openPhase = {...(S._openPhase||{}), [id]:ph.id}; saveNow(); reopen(); setTimeout(()=>{ const n = document.querySelector(`#panel .phase[data-phase="${ph.id}"] .phase-name .ed`); if(n){ beginEdit(n); n.querySelector('input')?.select(); } },60); };
  ...
  pn.querySelectorAll('[data-phdel]').forEach(b => b.onclick = e => { e.stopPropagation(); const ph = byId(p.phases, b.dataset.phdel); requestDelete({label:`${ph.name} (${projectPhaseTasks(p, ph).length} tasks)`, node:b.closest('.phase'), remove:()=>{ spliceOut(p.phases, x=>x.id===ph.id); if(typeof projectListsSync === 'function') projectListsSync(); }, after:reopen}); });
  pn.querySelectorAll('.quick-task').forEach(inp => inp.addEventListener('keydown', e => { if(e.key!=='Enter') return; const t = inp.value.trim(); if(!t) return; const ph = byId(p.phases, inp.dataset.quick); if(p.movedTo){ ph.movedAt = ph.movedAt || new Date().toISOString(); const nt = Object.assign(newTask(t, ''), {listId:p.id, sectionId:ph.id, links:{projects:[p.id], skills:[]}}); if(typeof planTaskDefaults === 'function') planTaskDefaults(nt); S.tasks.push(nt); } else ph.tasks.push({id:uid(), text:t, done:false, dueDate:null}); S._openPhase = {...(S._openPhase||{}), [id]:ph.id}; saveNow(); sound('click'); reopen(); setTimeout(()=>document.querySelector(`#panel .quick-task[data-quick="${ph.id}"]`)?.focus(),60); }));
  pn.querySelectorAll('[data-tdone]').forEach(c => c.addEventListener('change', () => { const [phid,tid] = c.dataset.tdone.split(':'); const ph = byId(p.phases,phid); const t = ph.movedAt ? byId(S.tasks, tid) : byId(ph.tasks, tid); t.done = c.checked; if(ph.movedAt) t.doneAt = c.checked ? today() : null; saveNow(); sound(c.checked?'success':'click'); reopen(); }));
  pn.querySelectorAll('[data-tdue]').forEach(i => i.addEventListener('change', () => { const [phid,tid] = i.dataset.tdue.split(':'); const ph = byId(p.phases,phid); if(ph.movedAt){ const t = byId(S.tasks, tid); t.day = i.value || ''; } else { const t = byId(ph.tasks, tid); t.dueDate = i.value || null; } saveNow(); reopen(); }));
  pn.querySelectorAll('[data-tdel]').forEach(b => b.onclick = () => { const [phid,tid] = b.dataset.tdel.split(':'); const ph = byId(p.phases,phid); const t = ph.movedAt ? byId(S.tasks, tid) : byId(ph.tasks,tid); requestDelete({label:t.text, node:b.closest('.task'), remove:()=>ph.movedAt ? spliceOut(S.tasks, x=>x.id===tid) : spliceOut(ph.tasks, x=>x.id===tid), after:reopen}); });
...
/* the same cleaning a standalone stream gets, parts included — normIncomeNums
   lives with the rest of the income shape, in finance.js */
hooks.pnum = (pid) => { const p = byId(S.projects,pid); if(p){ normIncomeNums(p.income); saveNow(); if(typeof finLiveRecalc === 'function') finLiveRecalc(); } };
function openNodModal(projectId, after, existing=null){
  ...
  const m = openModal(`<h2>${existing?'Edit nod':'A nod'}</h2><p class="muted" style="margin-top:-8px">${existing?fmtDate(existing.date,'med'):'I showed up and did this.'}</p><div class="stack">
    ...
    ${linkBoxHTML(`<input class="inp" id="nodLink" placeholder="link (optional) — or drop an image below" value="${esc(existing?.link||'')}">`, existing?.link || '')}${existing?`<div class="field"><label>Date</label><input class="inp" type="date" id="nodDate" value="${existing.date}"></div>`:''}
