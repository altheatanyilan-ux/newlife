...
function jzeSetDuration(s, dn){
  ...
  n.type = d.type; n.duration = d.units + (n.dotted ? Math.floor(d.units / 2) : 0);
  return true;
}
function jzeToRest(s){
  ...
  if(!n || n.isRest) return false;
  jzeSnap(s);
  n.isRest = true; n.pitch = null; n.isChord = false;
  return true;
}
function jzeToggleDot(s){
  ...
  if(!n) return false;
  jzeSnap(s);
  const base = jzeDurByType(n.type).units;
  n.dotted = !n.dotted;
  n.duration = n.dotted ? base + Math.floor(base / 2) : base;
  return true;
}
...
function jzePlace(s, letter, asChord){
  ...
  const note = {pitch: midi, duration: d.units, type: d.type,
    isRest: letter === null, isChord: !!asChord, dotted: false, tied: false};
  const at = notes[s.sel.ni];
  if(asChord && at && !at.isRest){
    notes.splice(s.sel.ni + 1, 0, note);
    s.sel.ni += 1;
    return true;
  }
  ...
  if(s.sel.ni + 1 < notes.length) s.sel.ni += 1;
    ...
    isChord:false, dotted:false, tied:false}); s.sel.ni += 1; }
  return true;
}
