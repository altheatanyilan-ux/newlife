...
function mediaLifeStage(e){ return stageForDate(mediaDateConsumed(e)); }
...
function renderLibraryTimeline(root){
  ...
  root.querySelectorAll('[data-retro-add]').forEach(b=>b.onclick=()=>{ const s = byId(S.stages,b.dataset.retroAdd); const r = stageYearRange(s); openMediaModal({finishedAt: r ? `${r[0]}-06-15` : ''}); });
}
...
function openMediaPanel(id){
  ...
  const p = openPanel(`<div class="mono">${k[0]} ${k[1].toLowerCase()} · ${MEDIA_STATUS_LABEL[x.status]}${stage?` · consumed during ${stage.char} ${esc(stage.name)}`:''}</div>
    ...
    <div class="vp-sec"><span class="sc">Linked dimensions</span><div class="faint" style="font-size:.78rem;margin-bottom:8px">Values touched (±), threads activated, visions fed, skills informed.</div>${linksEditorHTML(e.links)}</div>
  ...
  bindLinksEditor(p, e.links, () => saveNow());
