...
function entryCard(e, {clamp:cl=true, tools=true}={}){
  ...
  return `<article class="entry rv" data-entry="${e.id}">
    <div class="meta"><span class="mono">${typeIcon(e.type)} ${typeName(e.type)}</span><span class="mono">${esc(fmtDate(e.occurredAt,'med'))}</span>${e.confidence?`<span class="status-pill">${esc(e.confidence)}</span>`:''}${tools?`<span class="tools"><button class="tbtn" data-edit="${e.id}">edit</button>${['memory','lifeevent','reflection','decision'].includes(e.type)?`<button class="tbtn" data-promote-turn="${e.id}" title="make this a turning point in the Chronicle">→ turning point</button>`:''}<button class="snip-btn" data-snip="${e.id}" title="save to the Writing Studio">✂</button></span>`:''}</div>${tools?`<button class="del-x" data-del="${e.id}" title="delete" aria-label="delete entry">×</button>`:''}
