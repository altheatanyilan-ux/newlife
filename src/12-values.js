...
routes.values = function(root){
  ...
  root.innerHTML = `<div class="page">
    ...
    <div class="grid c2" style="align-items:start">
      <div class="card rv"><div class="row between"><h3>Priority order</h3><span class="mono">drag to re-rank · ${S.valueOrderHistory.length} re-rankings</span></div>
        <ul class="values-list" id="valuesList">${S.valueOrder.map((id,i)=>{ const v = byId(S.values,id); const c = valueCurrent(id); return `<li draggable="true" data-vid="${id}"><span class="rank">${i+1}</span><span class="nm"><a href="#/value/${id}" style="color:${v.color}">${esc(v.name)}</a></span><div class="bar" style="--c:${v.color}"><i style="width:${c}%"></i></div><span class="pct" data-tween="${c}" data-suffix="%">0</span><span class="row" style="gap:4px"><span class="faint">⋮</span><button class="del-x inline" data-valdel="${id}" title="delete value">×</button></span></li>`; }).join('')}</ul>
  ...
  $$('[data-valdel]',root).forEach(b => b.onclick = e => { e.stopPropagation(); deleteValue(byId(S.values, b.dataset.valdel), b.closest('li')); });
  // drag to reorder
...
async function openSnapshotModal(after){
  ...
  const m = openModal(`<h2>Congruence snapshot</h2><p class="muted">0–100 for each value. Not aspiration — where you actually are, this week.${last?` Sliders start where you left them on ${fmtDate(last.date,'med')}; move one and a note opens beneath it.`:''}</p>
    <div class="snapshot-form">${S.valueOrder.map(id=>{ const v=byId(S.values,id); return `<div class="sv-block" data-svb="${id}" style="--c:${v.color}"><div class="r"><span class="n" style="color:${v.color}">${esc(v.name)}</span><input type="range" class="slider" min="0" max="100" value="${base[id]}" data-sv="${id}" style="--c:${v.color}"><span class="mono" data-svl="${id}">${base[id]}</span></div>
      <div class="sv-note" data-svn="${id}" style="height:0" aria-hidden="true"><textarea class="ta" rows="2" data-svt="${id}" placeholder="What's driving this score today?" disabled tabindex="-1"></textarea></div></div>`; }).join('')}</div>
...
routes.value = function(root, params){
  ...
  root.innerHTML = `<div class="page narrow">
    ...
    <div class="row" style="margin-top:40px;justify-content:flex-end"><button class="btn sm ghost danger" id="delValue">delete this value</button></div>
  </div>`;
