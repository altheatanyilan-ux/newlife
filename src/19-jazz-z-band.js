...
function jazzTunePlaySettings(t){
  ...
  const d = {bpm: typeof jazzTempoOf === 'function' ? jazzTempoOf(t) : 120, choruses: 2, trading: false,
    ...
    soloist: 'sax', soloDensity: 'medium', soloOutside: 1, soloSeed: 1, soloVol: 1, startAt: 'solos'};
...
function jazzTuneTimeline(t, key, s){
  ...
  plan.forEach((kind, ci) => {
    ...
    /* the soloist improvises in the solo choruses — and in the heads too
       when the tune carries no written melody for them */
    if((kind === 'solo' || !mel) && s.soloist && s.soloist !== 'none' && typeof jzsSolo === 'function'){
...
async function jzbtStart(root, t){
  ...
  const player = scorePlayer(tl, {bpm: s.bpm, swing: straight ? 0 : jzbSwing(s.swingRatio, s.bpm), countIn: s.countIn,
    volumes: {0: s.melody || 0, 1: 1, 2: 0.85, 3: 0.65, 4: s.soloVol == null ? 1 : s.soloVol}, muted: new Set((s.melody > 0 ? [] : ['p:0']).concat(s.soloist && s.soloist !== 'none' ? [] : ['p:4'])),
    onEnd: () => { if(_jzbt.run && _jzbt.run.player === player) jzbtStop(); }});
  /* with a soloist, ▶ goes straight to the solos unless asked for the head */
  const soloing = s.soloist && s.soloist !== 'none';
  const firstSolo = soloing && (s.startAt || 'solos') === 'solos' ? tl.perf.find(p => p.kind === 'solo') : null;
  if(!player.start(null, firstSolo ? firstSolo.q0 : null)){ toast('This browser cannot make sound.'); return false; }
  ...
  const frame = () => {
    ...
    if(pm && pm.i !== r.lastBar){
      ...
      const who = soloing ? ((JZS_SOLOISTS.find(x => x[0] === s.soloist) || [])[1] || s.soloist) : '';
      const said = pm.kind === 'head' ? 'Head' : pm.kind === 'head_out' ? 'Head out' : `Solos · chorus ${pm.soloN} of ${s.choruses}${who ? ' · ' + who : ''}`;
