...
function migrate(){ if(S.settings && ['map','home','compass'].includes(S.settings.home)) S.settings.home = 'today'; wipeDemoData(); if(typeof treeEnsure === 'function') treeEnsure(); /* There was a clause here that read `if(S.rehearsal && !S.rehearsal){ S.rehearsal
   ...
   on every boot: the whole Morning Theatre — the self-image script, the winning
   feeling, the chief aim, the vision board, every scene, every scripting, the thanks,
   the 21-day tracker and the section order — was thrown away and reseeded empty every
   time the page was opened. Nothing else is needed in its place: seed() already fills
   `rehearsal` when it is genuinely absent, three lines below. */
  if(typeof migrateEras === 'function') migrateEras(); const d = seed(); for(const k of Object.keys(d)) if(S[k]===undefined) S[k] = d[k]; if(typeof migrateLifeline === 'function') migrateLifeline(); if(typeof migrateEntryLinks === 'function') migrateEntryLinks(); if(typeof migrateSkillLevels === 'function') migrateSkillLevels(); if(typeof migrateSkillAbilities === 'function') migrateSkillAbilities(); if(typeof migrateProjects === 'function') migrateProjects(); if(typeof migratePlanning === 'function' && typeof projectListsSync === 'function'){ try { planState(); projectListsSync(); } catch(e){ console.warn('project lists', e); } } if(typeof migrateStages === 'function') migrateStages(); if(typeof migrateTasks === 'function') migrateTasks(); if(typeof migrateIdeas === 'function') migrateIdeas(); if(typeof migrateMedia === 'function') migrateMedia(); if(typeof migrateSkillFocus === 'function') migrateSkillFocus(); if(typeof migrateSkillCats === 'function') migrateSkillCats(); if(typeof migratePeople === 'function') migratePeople(); if(typeof migrateValuePractices === 'function') migrateValuePractices(); if(typeof migrateFinance === 'function') migrateFinance(); if(typeof migrateRhythm === 'function') migrateRhythm(); if(typeof migrateTheatre === 'function') migrateTheatre(); if(typeof migrateStillness === 'function') migrateStillness(); if(typeof migrateReviews === 'function') migrateReviews(); if(typeof migrateJournalTypes === 'function') migrateJournalTypes(); S.boards = Array.isArray(S.boards) ? S.boards : []; if(typeof migrateBoards === 'function') migrateBoards(); }
...
/* Living View stamps lv-empty on anything blank and the stylesheet hides it
   outright. That is right for reading and fatal for writing: every "+ add"
   button in the app pushes a blank row and then asks to edit it, so the row
   was hidden the instant it existed — and focus() on a display:none input
   silently does nothing, which is why those buttons looked broken. A field
   being edited is the one thing on the page you are certainly looking at, so
   it and every wrapper above it come back into view for the duration. */
function unhideForEdit(node){
  for(let n = node; n && n !== document.body; n = n.parentElement) n.classList?.remove('lv-empty');
}
