...
function organicSVG(W, H){
  ...
  (() => {
    ...
    for(let i = 0; i < many; i++){
      ...
      const sz = (5.4 + rnd() * 4.2) * k, rot = rnd() * 360;
      const cl = lerpColor(catColor(host.cat), '#6f9a58', .45 + rnd() * .3);
      g += `<g class="leaf amb" data-phase="${(rnd()*6.28).toFixed(2)}" data-period="${(3 + rnd()*2.4).toFixed(2)}"
        transform="translate(${cx.toFixed(1)},${cy.toFixed(1)}) rotate(${rot.toFixed(0)})">
        <path d="M0,0 Q${sz.toFixed(1)},${(-sz*.7).toFixed(1)} ${(sz*2).toFixed(1)},0 Q${sz.toFixed(1)},${(sz*.7).toFixed(1)} 0,0"
          fill="${cl}" opacity="${(.26 + rnd()*.2).toFixed(2)}"/></g>`;
    }
  })();
  ...
  items.filter(i => i.kind==='cat').forEach(it => {
       ...
       half joins the two up, so the crown reads as one canopy. */
    {
      ...
      for(let q=0;q<m;q++){
        const t = .48 + (q/Math.max(m-1,1))*.5; const p = qp(it.start, it.ctrl, it.end, t);
        const tan = -qt(it.start, it.ctrl, it.end, t)*180/Math.PI; const sd = q%2 ? 1 : -1;
        const size = 8.2*k*(.72 + ((q*7)%6)/12); const rot = tan + sd*(46 + ((q*11)%18));
        bl += `<g transform="translate(${P(p)}) rotate(${rot.toFixed(1)})"><g class="leaf" data-phase="${((q*2.3)%6.28).toFixed(2)}" data-period="${(3.1 + (q*.41)%2).toFixed(2)}"><path d="M0,0 Q${size.toFixed(1)},${(-size*.7).toFixed(1)} ${(size*2).toFixed(1)},0 Q${size.toFixed(1)},${(size*.7).toFixed(1)} 0,0" fill="${lerpColor(col,'#6f9a58',.4)}" opacity=".72"/></g></g>`;
      }
  ...
  items.filter(i => i.kind==='skill').sort((a,b)=>a.depth-b.depth).forEach(it => {
    ...
    let lf = '';
    for(let i=0;i<n;i++){
      const t = .22 + (i/Math.max(n-1,1))*.76; const p = qp(it.start, it.ctrl, it.end, t); const tan = -qt(it.start, it.ctrl, it.end, t)*180/Math.PI; const sd = i%2 ? 1 : -1;
      /* size varies leaf to leaf so the canopy is not a stencil, but never with level */
      const size = 9.4 * k * (1 - wither*.3) * (.78 + ((i*7)%6)/12); const rot = tan + sd*(40 + ((i*13)%20)); const curl = wither ? ` Q${(size*.6).toFixed(1)},${(size*.35*sd).toFixed(1)} ${(size*.8).toFixed(1)},${(size*.5).toFixed(1)}` : '';
    ...
    /* ---- the flowers ----
       These were 5–8px across on a 600px tree: aphids, not blossoms. They are
       the thing the eye should go to, so they are drawn at a size that reads —
       and their shape carries the level rather than only their number.

         a bud at one, two petals closed
         opening at two, three petals coming apart
         a bloom at three, five petals and a centre
         a full bloom at four, seven petals with a shadow behind
         and at mastery eight petals in two rings with a gold heart

       The size is held in screen pixels rather than multiplied by the fit
       scale, because a flower that shrinks as the tree grows defeats the whole
       point of drawing it larger. */
    /* Fewer, larger. Six small flowers on a twig read as a smudge; two or three
       big ones read as a flowering branch, and the size already carries the
       level so the count does not have to. */
    const flowers = (s.planned || locked) ? 0 : clamp(Math.round(1 + prog * 2.4), 1, 4);
    if(flowers && lvl > 0){
      const tier = clamp(Math.ceil(prog * 5), 1, 5);
      const R = [0, 12, 15, 19, 23, 28][tier];
      const petalN = [0, 2, 3, 5, 7, 8][tier];
      const heart = lerpColor('#e8c7a0', '#d4a44c', prog);
      const petalCol = lerpColor('#efd0d8', col, .22);
      for(let i=0;i<flowers;i++){
        const t = .40 + (i/Math.max(flowers-1,1))*.56; const p = qp(it.start, it.ctrl, it.end, t);
        /* off the twig by most of their own width, alternating sides, so two
           neighbours never sit on top of each other */
        const sd = i%2 ? 1 : -1; const cx = X(p[0]) + sd*(R*.72), cy = Y(p[1]) - (R*.5);
        const spin = (i*37) % (360/petalN);
        const step = 360/petalN;
        const ring = (rr, op, cl) => Array.from({length: petalN}, (_, q) => {
          const ang = q*step + spin;
          const a = ang * Math.PI/180;
          return `<ellipse cx="${(Math.cos(a)*rr*.58).toFixed(2)}" cy="${(Math.sin(a)*rr*.58).toFixed(2)}"
            rx="${(rr*.58).toFixed(2)}" ry="${(rr*.40 * (1 + ((q*5)%3)*.05)).toFixed(2)}"
            transform="rotate(${ang.toFixed(0)})" fill="${cl}" opacity="${op}"/>`; }).join('');
        /* a bud is closed: two petals overlapping, no centre showing */
        const body = tier === 1
          ? `<ellipse rx="${(R*.58).toFixed(2)}" ry="${(R*.88).toFixed(2)}" fill="${petalCol}" opacity=".9"/>
             <ellipse rx="${(R*.34).toFixed(2)}" ry="${(R*.7).toFixed(2)}" fill="${lerpColor(petalCol,'#ffffff',.25)}" opacity=".85"/>`
          : tier >= 5
            /* two rings, the outer wider and paler, and a gold heart */
            ? `${ring(R, '.6', lerpColor(petalCol,'#ffffff',.35))}
               <g transform="rotate(${(step/2).toFixed(0)})">${ring(R*.72, '.95', petalCol)}</g>
               <circle r="${(R*.26).toFixed(2)}" fill="${heart}"/>
               <circle r="${(R*.13).toFixed(2)}" fill="#fff3d0" opacity=".8"/>`
            : `${ring(R, '.93', petalCol)}<circle r="${(R*.24).toFixed(2)}" fill="${heart}"/>`;
        lf += `<g class="blossom t${tier} ${blossom ? 'soon' : ''}" style="--d:${(i*.31).toFixed(2)}s"
          transform="translate(${cx.toFixed(1)},${cy.toFixed(1)})">
          ${tier >= 4 ? `<circle r="${(R*.9).toFixed(2)}" fill="${col}" opacity=".1"/>` : ''}${body}</g>`;
      }
    }
    ...
    if(mastered){ for(let i=0;i<3;i++){
      ...
      const cx = X(p[0]) + sd*8*k, cy = Y(p[1]) + 7*k, r = 7.4*k;
    ...
    lblItems.push({id:s.id, ex:X(it.end[0]), ey:Y(it.end[1]), x:X(it.end[0]) + outward*17, y:Y(it.end[1]), anchor:outward>0?'start':'end', w:Math.max(name.length*fs*.56, sub.length*8.5*.62) + 6, h:24, name, sub, fs, col});
