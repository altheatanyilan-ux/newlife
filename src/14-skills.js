...
function organicSVG(W, H){
  ...
  cats.forEach(c => {
    ...
    /* A category name used to be written here, at the branch tip, and entered
       the relaxation below marked `fixed` — which meant a skill label would
       move out of its way but two category names would not move out of each
       other's. With half a dozen categories on a tree grown from whatever ids
       the skills happen to have, that came out overlapping about half the
       time: "Intellectual" printed through "Social". It is written after the
       relaxation now, like every other label, and it can move like one. */
    lblItems.push({id: 'cat:' + c, ex: tip[0], ey: tip[1] + 8, x: lx, y: tip[1] + 8, anchor,
      w: Math.max(c.length * 7.2, 62) + 10, h: 24,
      cat: true, name: c, sub: `${count} skill${count === 1 ? '' : 's'}`, col});
  });
  ...
  lblItems.forEach(l => { if(l.fixed) return;
    ...
    /* a category name belongs with the branches rather than in the layer of
       skill labels over them, so it goes back into `labels` where it was */
    if(l.cat){
      labels += `${leader}<text class="sk-catlbl" x="${l.x.toFixed(1)}" y="${(l.y - 4).toFixed(1)}" text-anchor="${l.anchor}" style="fill:${l.col}">${esc(l.name)}</text>`
        + `<text class="sk-catsub" x="${l.x.toFixed(1)}" y="${(l.y + 8).toFixed(1)}" text-anchor="${l.anchor}">${esc(l.sub)}</text>`;
      return;
    }
