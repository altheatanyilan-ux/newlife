...
The Jazz Studio hears a real piano — an acoustic one through the microphone, or a MIDI keyboard — as one stream of notes (`#/jazz/piano`). Everything is analysed on this device; no audio leaves it, and a take's sound is kept only if you choose to keep it (with a **Delete take** beside it).

**Play it — live feedback.** Under every engraved exercise a strip listens while you play. Noteheads are marked as they are judged (green played, amber right at the second try, red a wrong note in its place, grey missed) and a wrong note appears as a small red ghost where it would sit on the staff; under each staff a timing lane shows each onset early or late. **Wait for me** holds the cursor until the step is right; **Play along** counts you in and moves with the click. A keyboard shows the expected keys outlined, what you played filled green or red, and what is missing pulsing. What it judges (exact notes, note names, a line in order, the rhythm, or nothing — free playing is described, not judged) and how strictly (lenient ±150 ms, standard ±80 ms, strict ±40 ms) are set per exercise from its kind and stage. After each attempt: accuracy, a timing score, the bars with the most errors and **Loop the tricky bit**, and what the kind of exercise is measured by — the swing ratio of a swing line, the evenness and tempo of a scale, which beat of a comping pattern you rush. Attempts are kept key by key; three whole attempts in a key at Standard with a timing score of 80 or more mark the key as yours and suggest raising your comfort. The practice plan reads them: a key whose last heard attempt failed comes round again. Twelve-key flashcards turn over when they hear the chord, and say how long it took; the lead-sheet cards take the microphone as well as MIDI.

**Transcribe.** With nothing expected, a take is written down after it is recorded, in a Worker: onsets, then what each onset struck from the spectrum that *rose* at it (so notes still ringing do not count), explained note by note by harmonic summation, then note ends, then clean-up (short or unsure notes dropped, a ringing note re-detected joined back, octave and twelfth shadows of a loud note removed). The trained models the request named (Onsets and Frames, Basic Pitch) fetch their weights over the network and need TensorFlow.js, so they are not used; the engine is replaceable, and `tools/transcribe.py` (ByteDance's piano_transcription_inference, which hears the pedal) is the optional local upgrade — its MIDI imports into Compose. On the test set's real performances played through the house's sampled grand, note F1 at ±50 ms is about 0.95 (Bach), 0.84 (Mozart), 0.85 (Haydn) and 0.68 (pedalled Chopin); the harness on the Piano input page measures it on recordings of your own piano, by register.

**Play to compose** (`#/jazz/compose`). Record with a click (the default, and the surest rhythm) or free tempo, from the microphone or a MIDI keyboard (pedal included). The clean-up is automatic: the beat (the click, or found by dynamic programming over the onsets — on steady playing it lands on the beat or a level of it), a grid chosen per beat between eighths, triplets and sixteenths with a charge for complication, swung eighths written straight under **Swing**, hands split by where each hand has been rather than a fixed line, a melody voice separated from the chords under it, the key found and every note spelled for the key and its chord, ties, rests and beams, chord symbols read off the harmony (rootless voicings included, ii–V–I preferred, unsure ones grey), pedal marks. A piano score or a lead sheet; takes appended bar-aligned; re-run the clean-up with another grid, swing or split; or click a note and delete it, move it (↑ ↓), lengthen, shorten or send it to the other staff — each hand correction counted. Exports **MusicXML 4.0**, compressed **.mxl** and **MIDI** (as written or as played); saves to Repertoire; can be kept as an exercise of your own that the Play it strip checks you on.

**Ear and reading** (`#/jazz/ear`). An aural test (a phrase played twice, then played back and aligned note by note, harder with your stage), sight-reading (an unseen melody — over changes from the middle stages — shown for a set time, then played in time and scored), and **play what you sing** (the voice heard by a YIN pitch detector, then the piano, compared by note name).

**Rubato profiles.** In Repertoire, **● Record a performance** records you at the piano and follows it like any synced recording, keeping its timing and dynamics as the piece's performance memory.
...
A violin or a cello part in Repertoire is played on real solo instruments recorded for sampling, both dedicated to the public domain (CC0 1.0): the solo violin of [VSCO-2 Community Edition](https://github.com/sgossner/VSCO-2-CE) (Versilian Studios), recorded with vibrato, and the [Karoryfer x Bigcat open-source cello](https://github.com/sfzinstruments/karoryfer-bigcat.cello), a solo cello recorded without vibrato. `node tools/fetch-orchestra.js --set strings` fetches them into `vendor/strings/` (every note the libraries recorded, two dynamic layers, up to seven seconds, 32 kHz); `build.js` embeds them. The player picks the soft recording up to mezzo-forte and the loud one from forte, lets the bow swell in and come off, gives the cello a vibrato that begins a moment after the note, and sends both to a quiet room (a reverb whose impulse is computed in the page, nothing fetched). The other orchestral instruments come from the CC0 set in `vendor/orchestra-lite/`.

In the Library, a link to a work is shown as what it is. Paste a Spotify, Apple Music, Apple Podcasts, YouTube, Vimeo, SoundCloud, Bandcamp, TIDAL, Deezer, Open Library, Goodreads, Wikipedia, IMDb, Letterboxd or Steam address — into "Log a work" or into a work's links — and it is recognised from the address alone (the service, the kind of thing, often the title), then shown as a card with its cover. For the cover and the title the page asks that service once, at the moment of pasting, through the service's own public preview address (oEmbed, or Open Library's and Wikipedia's open catalogues) — no third party, no account, nothing sent but the address — and keeps the answer with the work, the cover as a small picture of its own, so the card is whole offline and nothing is asked again unless you press ↻. A service that does not answer a web page is still a card from what its address says, with 🖼 for a cover of your own; an ordinary page stays a plain link. ▶ opens the service's own player in the panel, loaded only when pressed (`src/18-media-preview.js`, `smoke260.js`).

A symphony or a concerto is played on the orchestra in `vendor/orchestra-hq/` — the string sections, woodwinds and brass of VSCO-2 (CC0), fetched by `node tools/fetch-orchestra.js --set orchestra-hq`: both dynamic layers, 32 kHz, five-second notes, and the short notes each section recorded (spiccato, staccato). Where it carries an instrument, the build leaves out that instrument's thin copy. Once a score is an orchestra (winds, brass or timpani beside three or more string parts, or seven parts or more), its violin and cello parts are played by the sections and the soloist stays one instrument (the part named solo, or the one unnumbered violin among numbered ones); part names are read in English, Italian, German and French. Each part sits in its seat across the stereo field, every part sends into one hall the orchestra shares (computed in the page, nothing fetched), a tutti is trimmed so it stays under full scale, and a limiter just below full scale guards the rest. The score's own markings are played: *pizz.* plucks until *arco*, a staccato or a quick unslurred note comes from the short recordings, a slurred run stays joined, and a hairpin moves the loudness of the notes under it. `tests/fixtures/concerto-excerpt.js` is eight bars written to hear all of it; `smoke259.js` makes the claims.
